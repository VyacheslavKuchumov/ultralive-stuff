...
const createEquipment = async (req, res) => {
    try {
        const {
            equipment_name, serial_number, equipment_set_name, warehouse_name, needs_maintenance, date_of_purchase, cost_of_purchase
        } = req.body; // Add all required fields

        const foundWarehouse = await warehouse.findOne({ where: { warehouse_name: warehouse_name } });
        const foundEquipmentSet = await equipment_set.findOne({ where: { equipment_set_name: equipment_set_name } });
        ...
        if (foundWarehouse && foundEquipmentSet) {
            ...
            return res.status(500).send({ message: "Invalid warehouse or equipment set." });
        }
    } catch (error) {
        return res.status(500).send({ message: error.message });
    }
...
const editEquipmentById = async (req, res) => {
    try {
        ...
        const {
            equipment_name, serial_number, equipment_set_id, place_of_storage, current_place_of_storage, needs_maintenance, date_of_purchase, cost_of_purchase
        } = req.body;
        ...
        await equipmentToUpdate.update({
            equipment_name,
            serial_number,
            equipment_set_id, // Directly update using IDs
