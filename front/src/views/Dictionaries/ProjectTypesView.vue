<template>
    <v-container width="600">
        ...
        <v-col cols="12">
          ...
          >
            ...
            <template v-slot:item.actions="{ item }">
              ...
              <v-btn icon @click="deleteItem(item.project_type_id)">
      ...
      <v-dialog v-model="dialog" max-width="500px">
        <v-card>
          <v-card-title>
            <span class="headline">{{ form.project_type_id ? 'Edit' : 'Add' }} Project Type</span>
          </v-card-title>
          <v-card-text>
            <v-text-field v-model="form.project_type_name" label="Project Type Name" required></v-text-field>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
  ...
  export default {
    data() {
      return {
        ...
        headers: [
          { title: 'Name', value: 'project_type_name', width: 'auto' },
    ...
    methods: {
      ...
      openDialog(item = null) {
        this.form = item ? { ...item } : { project_type_id: null, project_type_name: '' };
        this.dialog = true;
      },
      closeDialog() {
        this.dialog = false;
