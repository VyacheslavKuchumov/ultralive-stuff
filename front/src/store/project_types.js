...
export default {
  ...
  actions: {
    ...
    async getProjectTypeByID({ commit }, project_type_id) {
      try {
        const response = await instance.get(`/api/project_types/${project_type_id}`);
    ...
    async updateProjectType({ commit }, projectType) {
      try {
        const response = await instance.put(`/api/project_types/${projectType.project_type_id}`, projectType);
        if (response) return commit('setProjectTypes', response.data);
      } catch (error) {
    ...
    async deleteProjectType({ commit }, projectType) {
      try {
        const response = await instance.delete(`/api/project_types/${projectType.project_type_id}`);
        if (response) return commit('setProjectTypes', response.data);
      } catch (error) {
